...
--   defines type classes whose names start with @Has...@, for which no new 
--   instances can be created. (They could be called phantom type classes.) 
--   For example, the mouse pointer coordinates can be retrieved using the 
--   function 'eventCoordinates' which requires that the first type parameter 
--   of 'EventM' is in the class 'HasCoordinates'. The module supplies instance 
--   of class 'HasCoordinates' for the types 'EButton', 'ECrossing', 'EMotion' 
--   and 'EScroll'. Thus for all events that require an 'EventM' action with 
--   one of the types above, the accessor function 'eventCoordinates' may be used.
--
...
import Graphics.UI.Gtk.Gdk.InputDevice (AxisUse(..))
...
import Graphics.UI.Gtk.Types ( DrawWindow, mkDrawWindow, Device, mkDevice )
...
-- | Class of events which have @(x, y)@ coordinates. No new instances can be
--   written, as the required @HasCoordinatesInternal@ class is not exported.
class (HasCoordinatesInternal a) => HasCoordinates a
instance (HasCoordinatesInternal a) => HasCoordinates a

class HasCoordinatesInternal a
instance HasCoordinatesInternal EButton
instance HasCoordinatesInternal EScroll
instance HasCoordinatesInternal EMotion
instance HasCoordinatesInternal ECrossing
...
-- | Class of events which have @(x, y)@ coordinates relative to the root 
--   (origin) of the screen. No new instances can be written, as the required 
--   @HasRootCoordinatesInternal@ class is not exported.
class (HasRootCoordinatesInternal a) => HasRootCoordinates a
instance (HasRootCoordinatesInternal a) => HasRootCoordinates a

class HasRootCoordinatesInternal a
instance HasRootCoordinatesInternal EButton
instance HasRootCoordinatesInternal EScroll
instance HasRootCoordinatesInternal EMotion
instance HasRootCoordinatesInternal ECrossing
...
-- | Class of events which have modifier keys. No new instances can be written, 
--   as the required @HasModifierInternal@ class is not exported.
class (HasModifierInternal a) => HasModifier a
instance (HasModifierInternal a) => HasModifier a

class HasModifierInternal a
instance HasModifierInternal EKey
instance HasModifierInternal EButton
instance HasModifierInternal EScroll
instance HasModifierInternal EMotion
instance HasModifierInternal ECrossing
...
-- | Class of events which have an input device. No new instances can be 
--   written, as the required @HasDeviceInternal@ class is not exported.
class (HasDeviceInternal a) => HasDevice a
instance (HasDeviceInternal a) => HasDevice a

class HasDeviceInternal a
instance HasDeviceInternal EButton
instance HasDeviceInternal EScroll
instance HasDeviceInternal EMotion
instance HasDeviceInternal EProximity

-- | The input device that generated the event.
eventDevice :: (HasDevice t) => EventM t Device
eventDevice = do
  ptr <- ask
  liftIO $ do
    (ty :: #{gtk2hs_type GdkEventType}) <- peek (castPtr ptr)
    if ty `elem` [ #{const GDK_BUTTON_PRESS},
                   #{const GDK_2BUTTON_PRESS},
                   #{const GDK_3BUTTON_PRESS},
                   #{const GDK_BUTTON_RELEASE}] then
        makeNewGObject mkDevice (#{peek GdkEventButton, device} ptr)
      else if ty `elem` [ #{const GDK_SCROLL} ] then 
        makeNewGObject mkDevice (#{peek GdkEventScroll, device} ptr)
      else if ty `elem` [ #{const GDK_MOTION_NOTIFY} ] then 
        makeNewGObject mkDevice (#{peek GdkEventMotion, device} ptr)
      else if ty `elem` [ #{const GDK_PROXIMITY_IN},
                          #{const GDK_PROXIMITY_OUT}] then
        makeNewGObject mkDevice (#{peek GdkEventProximity, device} ptr)                  
      else error ("eventDevice: none for event type " ++ show ty)

-- | Class of events which have a timestamp. No new instances can be written, 
--   as the required @HasTimeInternal@ class is not exported.
class (HasTimeInternal a) => HasTime a
instance (HasTimeInternal a) => HasTime a

class HasTimeInternal a
instance HasTimeInternal EKey
instance HasTimeInternal EButton
instance HasTimeInternal EScroll
instance HasTimeInternal EMotion
instance HasTimeInternal ECrossing
instance HasTimeInternal EProperty
instance HasTimeInternal EProximity
#if GTK_CHECK_VERSION(2,6,0)
instance HasTimeInternal EOwnerChange
#endif
...
foreign import ccall "gdk_event_get_axis" 
    gdk_event_get_axis :: Ptr evt -> CInt -> Ptr CDouble -> IO CInt

-- | Query the value of an extended input device axis.
eventGetAxis :: AxisUse -> EventM any (Maybe Double)
eventGetAxis ax = do
    ReaderT $ \ev -> alloca $ \valPtr -> do
            found <- liftM toBool $ gdk_event_get_axis (castPtr ev) (fromIntegral (fromEnum ax)) valPtr
            if found 
              then do
                liftM (Just . realToFrac) $ peek valPtr
              else do
                return Nothing

-- | Retrieve the pressure reported by an extended input device.
eventPressure :: EventM any (Maybe Double)
eventPressure = eventGetAxis AxisPressure

-- | Retrieve the wheel position reported by an extended input device.
eventWheel :: EventM any (Maybe Double)
eventWheel = eventGetAxis AxisWheel

-- | Retrieve the (x,y) coordinates reported by an extended input device.
eventDeviceCoordinates :: EventM any (Maybe (Double, Double))
eventDeviceCoordinates = do
    evx <- eventGetAxis AxisX
    evy <- eventGetAxis AxisY
    return $ liftM2 (,) evx evy

-- | Retrieve the (x,y) tilt reported by an extended input device.
eventTilt :: EventM any (Maybe (Double, Double))
eventTilt = do
    evx <- eventGetAxis AxisXtilt
    evy <- eventGetAxis AxisYtilt
    return $ liftM2 (,) evx evy
