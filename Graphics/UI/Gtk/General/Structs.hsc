...
import Graphics.UI.Gtk.Gdk.Enums (Function, Fill, SubwindowMode,
                                  LineStyle, CapStyle, JoinStyle, AxisUse)
#endif
...
-- | The 'DeviceAxis' type contains information about the range and mapping of a device axis.
data DeviceAxis = DeviceAxis AxisUse Double Double deriving (Eq,Show)

instance Storable DeviceAxis where
  sizeOf _ = #{const sizeof(GdkDeviceAxis)}
  alignment _ = alignment (undefined::#gtk2hs_type GdkAxisUse)
  peek ptr = do
    (use_ ::#gtk2hs_type GdkAxisUse) <- #{peek GdkDeviceAxis, use} ptr
    (min_ ::#gtk2hs_type gdouble) <- #{peek GdkDeviceAxis, min} ptr
    (max_ ::#gtk2hs_type gdouble) <- #{peek GdkDeviceAxis, max} ptr
    return $ DeviceAxis (toEnum $ fromIntegral use_) (realToFrac min_) (realToFrac max_) 
  poke ptr (DeviceAxis use_ min_ max_) = do
    #{poke GdkDeviceAxis, use} ptr ((fromIntegral $ fromEnum use_)::#gtk2hs_type GdkAxisUse)
    #{poke GdkDeviceAxis, min} ptr ((realToFrac min_)::#gtk2hs_type gdouble)
    #{poke GdkDeviceAxis, max} ptr ((realToFrac max_)::#gtk2hs_type gdouble)
